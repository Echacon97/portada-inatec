...
class PDFGenerator {
  constructor() {
    ...
    this.INATEC_LOGO_SRC = "inatec_logo.png"; // Ruta de la imagen del logo de INATEC
  }

  /**
   * Carga el logo de INATEC y lo convierte en un string Base64.
   * @returns {Promise<string>} - Una promesa que se resuelve con la cadena Base64 de la imagen.
   */
  async loadInatecLogo() {
    const img = new Image();
    img.src = this.INATEC_LOGO_SRC;
  ...
  async generate() {
    ...
    const center = document.querySelector("#centro").value;
    const career = document.querySelector("#carrera").value;
    const module = document.querySelector("#modulo").value;
    ...
    // Inserta el logo de INATEC
    const logoBase64 = await this.loadInatecLogo();
    this.doc.addImage(
      logoBase64,
      "PNG",
    ...
    // Configuración del módulo y tarea
    this.doc.setFontSize(18);
    this.writeCenteredText(module, line);
    ...
    this.doc.save("documento-inatec.pdf");
  }
}
